// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.13;

contract MessageMain {
    // event MessageReceived(bytes32 messageId, uint32 origin, bytes32 sender, bytes body);

    address mailbox; // address of mailbox contract

    bytes public messages;

    constructor(address _mailbox) {
        mailbox = _mailbox;
    }

    modifier onlyMailbox() {
        require(
            msg.sender == mailbox,
            "Only mailbox can call this function !!!"
        );
        _;
    }

    // handle function which is called by the mailbox to bridge messages from other chains
    function handle(
        uint32 _origin,
        bytes32 _sender,
        bytes memory callData
    ) external onlyMailbox {
        messages = callData;
    }

    // Custom decoding function for your specific callData structure
    function decodeCallData(
        bytes memory _callData
    ) external pure returns (bytes32, bytes memory, bytes memory, address) {
        (
            bytes32 uuid,
            bytes memory byteCode,
            bytes memory encodedFunctionData,
            address executeContractAddress
        ) = abi.decode(_callData, (bytes32, bytes, bytes, address));
        return (uuid, byteCode, encodedFunctionData, executeContractAddress);
    }
}
